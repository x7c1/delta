...
where
    ...
    W: Workspace,
{
    /// Dispatch the oldest `queued` send for a session, if one is recorded and
    ...
    /// quote). Once the session goes idle this promotes the send to `dispatched`
    ...
    ///
    /// A no-op when there is no queued send, or the session has no live pane
    /// (closed) — in which case the send stays `queued` and is dispatched the
