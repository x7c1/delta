...
///
/// Errors are returned (not panicked) so the scenario engine can decide what a
/// failed hook means; a real `claude` likewise carries on when a hook endpoint
/// is unreachable.
pub fn post_json<P: Serialize>(url: &str, payload: &P) -> Result<u16, String> {
    ...
    // Read the whole response (Connection: close ends it) and pull the status
    // out of the first line. The response body is irrelevant to the fake: the
    // only body the real `claude` consumes (UserPromptSubmit's
    // additionalContext) alters prompt text Delta never re-reads.
    let mut response = String::new();
    stream
        .read_to_string(&mut response)
        .map_err(|e| format!("read response from {url}: {e}"))?;
    parse_status(&response).ok_or_else(|| format!("malformed response from {url}"))
}
