...
fn map_session(row: &Row<'_>) -> rusqlite::Result<SessionParts> {
    Ok(SessionParts {
        ...
        repo_root: row.get(7)?,
        repository_display_name: row.get(8)?,
    })
}
...
const SESSION_COLS: &str = "id, cwd, transcript_path, title, status, created_at, \
     branch_at_launch, repo_root, repository_display_name";
...
impl SessionStore for SqliteStore {
    ...
    async fn insert_spawning_session(
        ...
        // `repository_display_name`) is written once here and never updated
        // later — see the doc on `Session`.
        conn.execute(
            "INSERT INTO session
