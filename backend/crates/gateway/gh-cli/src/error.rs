...
/// process, or `gh search prs` returning non-zero exit despite a
/// successful auth check) that should bubble up rather than be silently
/// swallowed.
#[derive(Debug, Error)]
pub enum Error {
    ...
    /// `gh search prs` exited non-zero. Its stderr is surfaced so a
    /// rate-limit / auth-rotated failure is debuggable from logs.
    ...
    /// `gh search prs --json` returned output that was not the expected
