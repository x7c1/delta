  ...
  queryClient.setQueryData<SendsResponse>(
    ...
    (previous) => {
      ...
      return {
        ...
        // The live state (turn phase, pending permission, pending question) is
        // server-reported; an optimistic insert learns nothing about it, so
        // keep what the last fetch said (or idle/none before any fetch) and let
        // the follow-up invalidation reconcile.
