 ...
 * not-yet-existing session. When the POST is accepted, focus switches from
 * the new-session state to the real session — and the chip must survive that
 * re-keying and stay visible until the scripted turn actually completes, not
 * vanish at the focus transition.
 *
...
test('the first-send pending chip stays visible until the scripted turn completes', async ({
  ...
  // The POST was accepted, so focus has already switched from the new-session
  // state to the real session (the new-session placeholder leaves). The chip
  // must still be there — the turn has not completed.
