            ...
            {selectedRepo.clones.map((clone) => {
              ...
              return (
                <li key={clone.path}>
                  <button
                    type="button"
                    // Committing a clone stamps `directory` provenance (see
                    // `setNewSessionWorkdir`), which drops any PR pick — so at
                    // most one row reads as the active pick across the three
