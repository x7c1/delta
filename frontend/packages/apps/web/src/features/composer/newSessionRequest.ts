 ...
 * defaults, `worktree` is omitted unless the opt-in was made, and `provider` is
 * omitted when it equals `PROVIDER_WIRE_DEFAULT` — the backend resolves an
 * omitted `provider` to exactly that, so omitting it keeps a send on the
 * default provider byte-for-byte identical to a pre-provider send.
 */
export function newSessionSendBody(
  text: string,
  { workdir, launchOptionIds, provider, worktree }: NewSessionLaunch,
): SendRequest {
  return {
    new_session: true,
    text,
